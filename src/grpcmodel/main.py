...
class GRPCModelMetaclass(ModelMetaclass, DeclarativeMeta):
    __grpcmodel_relationships__: Dict[str, RelationshipInfo]
    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        dict_used = {
            **dict_for_pydantic,
            "__weakref__": None,
            "__grpcmodel_relationships__": relationships,
    ...
    def __init__(
        ...
        if is_table_model_class(cls) and not base_is_table:
            for rel_name, rel_info in cls.__grpcmodel_relationships__.items():
...
_TGRPCModel = TypeVar("_TGRPCModel", bound="GRPCModel")


class GRPCModel(BaseModel, Message, metaclass=GRPCModelMetaclass, registry=default_registry):
    # SQLAlchemy needs to set weakref(s), Pydantic will set the other slots values
    __slots__ = ("__weakref__",)
    __tablename__: ClassVar[Union[str, Callable[..., str]]]
    __grpcmodel_relationships__: ClassVar[Dict[str, RelationshipProperty[Any]]]
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if name in {"_sa_instance_state"}:
            ...
            if name not in self.__grpcmodel_relationships__:
                super().__setattr__(name, value)
    ...
    def grpcmodel_update(
        ...
        *,
        ...
        if isinstance(obj, dict):
            ...
            raise ValueError(
                "Can't use grpcmodel_update() with something that "
                f"is not a dict or GRPCModel or Pydantic model: {obj}"
            )
        return self
    ...
    def serialize_partial_to_string(self, **kwargs: Any) -> bytes:  # type: ignore[override]
        ...
        # Same behavior; GRPCModel does not track required fields in protobuf sense
        return self.serialize_to_string(**kwargs)
    ...
    def merge_from(self, other_msg: Message) -> None:  # type: ignore[override]
        ...
        if isinstance(other_msg, GRPCModel):
    ...
    def register_extension(extension_handle: Any) -> None:  # type: ignore[override]
        ...
        # Not supported for dynamic GRPCModel-based messages
        return None

    @staticmethod
    def RegisterExtension(extension_handle: Any) -> None:  # type: ignore[override]
        return GRPCModel.register_extension(extension_handle)
    ...
    def set_in_parent(self) -> None:  # type: ignore[override]
        ...
        # No-op: presence is implicit in GRPCModel
        return None
    ...
    def which_oneof(self, oneof_group: str) -> Optional[str]:  # type: ignore[override]
        ...
        """
        # GRPCModel does not model oneof groups
        return None
